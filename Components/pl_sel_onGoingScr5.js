...
// raw code has comnted code etc
// has renderTeamA & renderTeamB has dynmc chng bg color brder color disable bg colr +icon tick icon
// works auto mode 
// works manual mode
// checks whoz turn exactly
...
const BothTeamOpponentSelection = ({
  ...
  useEffect(() => {
    ...
    //  const ws = new WebSocket('wss://qaws.player6sports.com/191/gameEvents/5847');
...
const renderTeamAItem = ({ item, index }) => {
        ...
        <View style={styles.plyrlft}>
          <View style={[styles.plyrprfl, styles.plyrlftprfl]}>
            <Image
              ...
              onError={() => console.error(`Failed to load image for ${item.name}`)}
            />
          </View>
  ...
  //   const getBorderColor = (item, global_6players, userInfo) => {
  //     if (global_6players.playerId === item.playerId) {
  //       if (userInfo?.userId === global_6players.userId) {
  //         return styles.borderTopBlueWidth;
  //       } else {
  //         return styles.borderTopOrangeWidth;
  //       }
  //     }
  //     return null; // Do nothing if the first condition fails
  //   };
