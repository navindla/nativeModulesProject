...
// icons, homeTeam , low Internet added tested working fine

// multi-sel players at once restricted, set true if button press
// onMsg--> hideProgrss

// 1st pick ,vacp gameid condtion added,  timer working
...
import { getWebSocketURL } from '../../../Services/Services';
...
const BothTeamOpponentSelection = ({
  ...
  useEffect(async () => {
    const isConnected =
      await Functions.getInstance().checkInternetConnectivity();
    if (!isConnected) {
    ...
    //   `wss://demows.player6sports.com/${userInfo?.userId}/gameEvents/${UserStore.selectedGameData.gameId}`,
    // );
    const ws = new WebSocket(
      getWebSocketURL(userInfo?.userId, UserStore.selectedGameData.gameId)
    );

    // `https://api.player6sports.com/`
    ...
    ws.onmessage = event => {
      ...
      try {
        ...
        if ( parsedData.event === "COMPLETED" && UserStore.selectedGameData.gameId === parsedData.data?.gameId) {
        ...
        if ( myStatusData && String(userInfo?.userId) === String(myStatusData.userId) ) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (totalSeconds <= 0) {
        ...
        //  totalSeconds = 30; // Reset to 30 seconds (00:30) //uncomnt
  ...
  const handlePlayerClick = (playerIndex, team, item) => {
    ...
    //////////////////////////////////////////// setmyTurn False to avoid multi touch
      ...
      .then(result => {
        ...
        if (result.status == 200) {
          ...
          Functions.getInstance().Toast(
            'error',
            'Unable to proceed,try again later',
          );
          navigation.navigate('BettingAddMoney');
        }
      });
      ...
      <View style={styles.countryBoxTopHeader}>
        <View style={styles.teamLeft}>
          ...
          <View style={styles.teamName}>
            <Text style={styles.teamLeftNameText}>{userPro?.name}</Text>
          </View>
        </View>
        ...
        <View style={styles.plyrlft}>
          ...
          >
            <Image
              ...
              source={{
                uri: `https://player6sports.s3.us-east-1.amazonaws.com/Players/${item.imageId}`,
              }}
              onError={e => {
            ...
            {item.playerRoleType === '0' && (
              <Image
                source={{
                  uri: 'https://player6sports.s3.us-east-1.amazonaws.com/pl6Assets/bat10.png',
                }}
            ...
            {item.playerRoleType === '2' && (
              ...
              <Image
                source={{
                  uri: 'https://player6sports.s3.us-east-1.amazonaws.com/pl6Assets/alrounder.png',
                }}
            ...
            {item.playerRoleType === '3' && (
              <Image
                source={{
                  uri: 'https://player6sports.s3.us-east-1.amazonaws.com/pl6Assets/cricketBall.png',
                }}
            ...
            {item.playerRoleType === '4' && (
              <Image
                source={{
                  uri: 'https://player6sports.s3.us-east-1.amazonaws.com/pl6Assets/wicketKeepr.png',
                }}
  ...
  if (myFinal.length === 6) {
    return (
      <ModalPopup
        text1={'Your final list is saving, please wait...'}
        text2={'Select your Captain & ViceCap in the next screen'}
        text3={'C = 2x runs VC = 1.5x runs'}
      />
    );
  }
