...
const BothTeamOpponentSelection = ({ navigation, showProgress, hideProgress }) => {
   // const [userInfo, setUserInfo] = useState(null);
    ...
    useEffect(() => {
        // Convert the initial time string into seconds
        ...
        //    teamBCount = teamBCount + 1;
            ...
            // Format the new time back into mm:ss
            const min = String(Math.floor(totalSeconds / 60)).padStart(2, '0');
            const sec = String(totalSeconds % 60).padStart(2, '0');
            setCurrentTime(`${min}:${sec}`);
          }
    
          // Stop timer when it reaches 00:00 or if stop is pressed
          if (totalSeconds <= 0 || !isRunning) {
            clearInterval(interval);
            setCurrentTime('00:00'); // Ensure the timer stops at 00:00
            setmyTurn(!myTurn);      // added 
           // setCurrentTime('00:30'); //added 
          }
        }, 1000);
      ...
      //  myTurn    // added
      ]); // Depend on isRunning to restart the timer on chang
