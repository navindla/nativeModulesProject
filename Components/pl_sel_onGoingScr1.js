//  has a seperate state. for pl 6 selected players list dummyData
// has modified renderTeamAItem() wth diisable button for list from global_6players compares with teamA state List
// has modified renderTeamBItem() wth diisable button for list from global_6players compares with teamB state List
    ...
    const renderTeamBItem = ({ item, index }) => {
        ...
        return (
            <View style={[styles.tmplyrlft,
                ...
                <TouchableOpacity 
                    style={[styles.adplyr,
                        { backgroundColor: isPlayerDisabled? 'pink' : 'white'
                        }
                    ]}
                    onPress={() => {
                        Functions.getInstance().fireAdjustEvent("pogpop");
