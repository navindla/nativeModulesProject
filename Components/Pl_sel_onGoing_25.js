    ...
    useCallback(() => {
      ...
      const setupWebSocket = async () => {
        ...
        ws.onmessage = (event) => {
          ...
          if (
            myStatusData &&
            String(userInfo?.userId) === String(myStatusData.userId)
          ) {
            console.log("My Status:--- ", myStatusData);
            setMyStatus(myStatusData);
            setMyFinal(myFinalData); // Update the state with myFinal data
        ...
        // Update globalList with both final data arrays below 2 line not worked out
        if (myFinalData || opFinalData) {
          setglobal_6players([...(myFinalData || []), ...(opFinalData || [])]);
        }
      ...
      return () => {
        if (ws) {
          ws.close();
          ws = null;  //  memory leaks
        }
      };
    }, [navigation]) // Add dependencies
  );
...
/////////
  ...
  const renderTeamAItem = ({ item, index }) => {
    ...
    const getBorderColor = (item, global_6players, userInfo) => {
      ...
      if (matchingPlayer) {
        return   matchingPlayer.userId === gameDetails.userOne // matchingPlayer.userId.toString() === gameDetails.userOne.toString()
