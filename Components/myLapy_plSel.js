...
const BothTeamOpponentSelection = ({
  ...
  useEffect(() => {
    const unsubscribe = navigation?.addListener('focus', async () => {
      try {
        ...
        // Fetch user data and reset AsyncStorage
        await fetchUserData();
        await AsyncStorage.setItem('p6data', 'false');
        await AsyncStorage.setItem('pl6-socket', 'false');
        callPlayer11Data(); // Uncommented
      } catch (error) {
    ...
    ws.onmessage = event => {
      try {
        ...
        if (myStatusData && userInfo?.userId === myStatusData.userId) {
          ...
          setglobal_6players(prev => [...prev, ...myFinalData, ...opFinalData]);
        } else if (opnStatusData) {
          setMyStatus(opnStatusData);
          setglobal_6players(prev => [...prev, ...opFinalData, ...myFinalData]);
        }

        // Check if global_6players length is 12 to navigate
        if (myFinalData.length + opFinalData.length === 12) {
    ...
    return () => {
      hideProgress();
      setLoading(false);
      newSocket.close(); // Ensure proper cleanup of any other sockets
      unsubscribe();
  ...
  }, [isRunning, currentTime]); // Depend on isRunning and currentTime

  // Update myTurn based on myStatus
  useEffect(() => {
    if (myStatus.need_to_pick !== undefined) {
      setmyTurn(myStatus.need_to_pick);
    }
  }, [myStatus.need_to_pick]); // Use specific property instead of the whole object

  // Effect to handle myTurn changes
  useEffect(() => {
    if (myTurn) {
      setLoading(false);
      ...
      setLoading(true);
      //  setCurrentTime('00:30'); // Ensure UI reflects the reset immediately uncmnted fri
    }
  }, [myTurn]);
