...
// 2nd screen: oppLoader also code in this file exists
...
// has bugs wrong color indication.
...
const LoadingPopup = ({global_6players, isVisible, closePopup, navigation, teamA, teamB, teamABench, teamBBench,isOpponent, userInfo, userStoreGdetails}) => {
  ...
  const renderTeamAItem = ({ item, index }) => {
       ...
       const getBorderColor = (item, global_6players, userInfo) => {
         ...
         if (matchingPlayer) {
           return matchingPlayer.userId === userStoreGdetails.userOne
             ? //matchingPlayer.userId === userInfo?.userId
             styles.borderTopOrangeWidth
              // styles.borderTopBlueWidth
             : styles.borderTopBlueWidth
