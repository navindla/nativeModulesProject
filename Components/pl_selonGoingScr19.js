...
//2.1.841: capable of doing multi-taskng
...
const BothTeamOpponentSelection = ({
  ...
  useEffect(() => {
    ...
    //     const myFinalData = parsedData.data?.myFinal ?? [];  // Default to empty array if undefined or null
    //     const opFinalData = parsedData.data?.opFinal ?? [];  // Default to empty array if undefined or null
...
//// cached respo async /////////////////////v1
  // const callPlayer11Data = async () => {
  //   try {
  //     showProgress();
      
  //     // Retrieve cached response from AsyncStorage
  //     const cachedResponse = await AsyncStorage.getItem("player6-api-response");
  ...
  //       await AsyncStorage.setItem("player6-api-response", JSON.stringify(result));
...
const callPlayer11Data = async () => {
  try {
    ...
    if (cachedResponse) {
      console.log("Using cached API response for gameId:", gameDetails.gameId);
      const result = JSON.parse(cachedResponse);
      handleApiResponse(result);
      hideProgress();
      return;
...
// Separate function to handle API response
const handleApiResponse = (result) => {
  console.log("API response 1sttt---:", result);
  setIsRunning(true); // start timer 00:30
  selectionTime = result.selStartTm;
  setTeamA(result.teamA11);
  setTeamB(result.teamB11);
...
////////////////////////////////////////
  ...
  return (
    ...
    {
      teamA.length === 0 ? (
        <Text style={{fontSize:12, color:'#fff', textAlign:'center',  marginTop:30}}>Loading..</Text>
      ) : (
