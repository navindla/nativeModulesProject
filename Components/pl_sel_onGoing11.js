...
// v10
//// added end time from wss sock
// bugs: need to assign end time at every instanmce
...
const BothTeamOpponentSelection = ({
  ...
  useEffect(() => {
    ...
    // sets state all req flds Orginal
    // ws.onmessage = (event) => {
    //   try {
...
//     const nextPickTime = new Date(myStatus.next_picked_player).getTime();
//     console.log('-----time nextPickTime---- ',nextPickTime);
//     const now = Date.now();
//     const secondsLeft = Math.floor((nextPickTime - now) / 1000);
//     console.log('-----time Left---- ',secondsLeft); //- value
    
//     // If time is up, set to "00:00"
//     if (secondsLeft <= 0) {
...
//     console.log('-----time setCurrentTime---- ',minutes,seconds);
...
// const calculateTimeLeft = () => {
// //  // const nextPickTimeStr = "2024-12-11 05:00:24"; // Example value
//   const nextPickTime = new Date(myStatus?.next_picked_player?.replace(" ", "T")).getTime(); // Convert to timestamp
//   const now = Date.now(); // Get current timestamp

//   // Calculate the absolute difference in seconds
//   const secondsLeft = Math.abs(Math.floor((nextPickTime - now) / 1000));

//   console.log('Next Pick Time API:', myStatus);
//  console.log('Current Time (ms):', now);
//   console.log('End Time :', nextPickTime);
//  console.log('Time Difference in seconds (absolute):', secondsLeft);
// };


const getCurrentTimeUTC = () => {
  if (!myStatus || !myStatus.next_picked_player) {
    console.warn("myStatus or next_picked_player is missing. Setting default value.");
    setCurrentTime("00:00"); // Set a fallback or handle as required
    return;
  }
  const nextPickTimeStr = myStatus?.next_picked_player // Example value "2024-12-11 06:23:37",
  ...
  // Parse the next pick time and current time as Date objects
  ...
  const timeDifferenceInSeconds = String(Math.floor((nextPickTime - nowTime) / 1000))  // Log the result
  console.log('Next Pick Time API:', nextPickTimeStr);
...
useEffect(
  ()=>{
    if (selectionProcess) {
      ...
      console.log(' moving...',selectionProcess);
      ...
      Services.getInstance()
        ...
        .then((result) => {
          ...
          if (!result.playerSelection) {
            console.log('----------navigate to runnMatchs------')
