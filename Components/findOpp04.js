...
`;
  ...
  const getData = async () => {
    console.log("getData called :::::::", UserStore.selectedMatch);
    showProgress();
    ...
    Functions.getInstance()
            ...
            .then((result) => {
              ...
              if (result.status === 401 && result.error === 'Low wallet Balance') {
               ...
               // navigation.goBack();
                ...
                if ( from ===  'Frnd' ){
                  UserStore.setHostAfrnd([...UserStore.hostAfrnd, { roomCode: couponCode, gameId: result.gameId }]);
                }
      ...
      useEffect(() => {
        ...
        if (GAMEid === 0) {
          Functions.getInstance().Toast("error", "Invalid Game ID");
          return;
        }
...
{
            UserStore.selectedMatch?.price === "0" ? (
              <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
              <Animated.View style={[LocalStyles.couponContainer,
                 animatedStyle
                 ]}>
                <Text style={LocalStyles.couponText}>{UserStore.selectedMatch?.roomCode}</Text>
                <TouchableOpacity
                  onPress={copyToClipboard}
                  style={LocalStyles.copyIconContainer}
                >
